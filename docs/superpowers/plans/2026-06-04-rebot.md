# rebot Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `rebot`, a Bun/TypeScript single-binary CLI that runs PR-Agent-like `describe`, `review`, `improve`, and `all` workflows through opencode.
...
```json
{
  ...
  "scripts": {
    ...
    "build": "bun build src/cli.ts --compile --outfile rebot"
  },
  "dependencies": {
...
git commit -m "chore: scaffold rebot typescript project"
```
...
import type { CliOptions, NormalizedInput, RebotCommand, RunResult } from "./types"
...
git commit -m "feat: add rebot cli orchestration"
```
...
```bash
./rebot review --diff-file fixtures/sample.patch
```
...
git commit -m "docs: document rebot usage and build"
```
...
- Type consistency: `CliOptions`, `NormalizedInput`, `RebotCommand`, and `RunResult` are defined in Task 1 and reused consistently by later tasks.
